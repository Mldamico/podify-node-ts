...
export const getAutoGeneratedPlaylist: RequestHandler = async (req, res) => {
  ...
  const playlistAutoGenerated = await AutoGeneratedPlaylists.aggregate([
    matchOptions,
    { $sample: { size: 4 } },
    {
      $project: {
  ...
  const playlist = await Playlists.findOne({ owner: req.user.id, title });

  const finalList = playlistAutoGenerated.push({
    id: playlist?._id,
    title: playlist?.title,
    itemsCount: playlist?.items.length,
  });
...
export const getPlaylistAudios: RequestHandler = async (req, res) => {
  ...
  if (!isValidObjectId(playlistId)) {
    return res.status(422).json({ error: "Invalid playlist id provided" });
  }
  ...
  const [playlistResult] = await Playlists.aggregate(aggregationLogic);

  if (!playlistResult) {
    const [autoPlaylistResult] = await AutoGeneratedPlaylists.aggregate(
      aggregationLogic
    );
